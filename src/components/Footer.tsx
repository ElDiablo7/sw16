...
export default function Footer() {
  ...
  return (
    <footer className="bg-gradient-to-br from-slate-900 to-[#243b53] text-gray-300 py-12 lg:py-16">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12 mb-12">
          
          {/* Company Info */}
          <div className="space-y-4">
            <Link href="/" className="flex items-center gap-1 inline-block">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {['Home', 'Get a Quote', 'Gallery', 'Reviews', 'FAQ', 'Contact'].map((item) => (
                <li key={item}>
                  <Link href={`/${item.toLowerCase().replace(/\s+/g, '-')}`} className="hover:text-[#1B8751] transition-colors">
                    {item === 'Home' ? 'Home' : item}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={service.name}>
                  <Link href={service.href} className="hover:text-[#1B8751] transition-colors">
                    {service.name}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                <MapPin className="w-5 h-5 text-[#1B8751] mt-1 flex-shrink-0" />
                <span>{ADDRESS}</span>
              </li>
              <li className="flex items-center gap-3">
                <Phone className="w-5 h-5 text-[#1B8751] flex-shrink-0" />
                <a href={PHONE_HREF} className="hover:text-white transition-colors">{PHONE_NUMBER}</a>
              </li>
              <li className="flex items-center gap-3">
                <Mail className="w-5 h-5 text-[#1B8751] flex-shrink-0" />
